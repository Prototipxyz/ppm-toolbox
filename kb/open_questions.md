...
| OQ-75 | GST welded status internally inconsistent: only 6/643 BOM rows marked Done at extraction, and 4 of those 6 are child rows whose parent row is NOT marked welded. Likely a shop-floor marking error rather than a true progress signal -- needs floor confirmation before being used in any status/cost decision. Surfaced building a trial GST status report (one-off Excel deliverable, not yet a KB pipeline artifact). | Data quality / floor confirmation |
| OQ-76 | Anon/publishable key (entities.md) returns `404 Bucket not found` on Storage REST calls (`/storage/v1/bucket/part-photos`, `/storage/v1/bucket/kb-private-fixtures`) -- but Voja confirms via Supabase Dashboard (authenticated session) that GST images ARE present in Storage. Discrepancy not yet root-caused: likely anon key lacking Storage list/read permission (RLS-on-storage gap) rather than wrong bucket name, but unconfirmed. Needs checking before any client-side (anon-key-driven) image feature is built -- app's eventual part-photo display will use this same key. | Before image features / Storage RLS |
