...
## PPM_ExportPartData Build Session — Open Items (OQ-78, OQ-79)

| OQ-78 | **REPORT sheet operation-coverage % — eligible-parts denominator has no data-model backing yet.** D-214 requires % calculations to use an eligible-parts denominator, not total-part count. PPM_ExportPartData's current data model has no "eligible for op X" concept distinct from the REQ flag itself — the macro computes % = (parts with REQ=1) / (total parts), i.e. coverage-of-all-parts, not true eligible-parts coverage. Needs a decision: acceptable for v1, or does eligibility need defining (e.g. per-material-type op applicability) before REPORT sheet ships. | Before REPORT sheet considered spec-complete |
| OQ-79 | **BOM_FLAT and PARTS sheet Level column hardcoded to 1 for all rows.** True assembly-hierarchy depth (multi-level BOM nesting) is not implemented in PPM_ExportPartData v1 — every row, regardless of actual nesting depth, gets Level=1. Needs a decision: acceptable simplification for pilot, or blocking issue requiring recursive level tracking during the assembly walk. | Before pilot use, if level data matters downstream |
